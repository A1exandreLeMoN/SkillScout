...
6. 非 2xx 或执行错误都要输出结构化 error_info，并在报告里单独列出。
7. 报告里要区分：业务问题、配置问题、runner 问题、执行层错误。
8. 最终是否完整，不看条数，只看 profile 声明的能力、action 和执行健康是否被覆盖。
9. 只要存在未被解释的执行层错误，最终评级不得是 Pass。
...
建议解释口径：
- 主流程不可用 或 高风险误导：`Fail`
- 存在执行层错误（网络、网关、插件、解析异常等）：至少 `Fail`
